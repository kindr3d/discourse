class Admin::StatsController < Admin::AdminController
  ...
  def index
    ...
    #TODO break in two see Max's suggestions
    visit_data = UserVisit.where("visited_at > ?", time_unit[:visit_limit]).group("DATE_TRUNC('#{time_unit[:unit]}', visited_at)").count.map {|k, v|
      ...
      top_categories = Topic.select("categories.id, categories.name, categories.color, count(topics.id) as thecount").joins(:category).where("topics.created_at > ?", time_unit[:current])
      ...
      render json: { visit_title: "Visits" , user_title: "Users", topic_title: "Topics", time: time, time_u: time_unit, visit_total: visits, visit_data: visit_data, user_total: users, user_data: user_data, topic_total: topics, topic_data: topic_data, topic_categoies: top_categories, post_total: posts, top_topics: top_topics}
